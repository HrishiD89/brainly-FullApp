...
import {TwitterIcon} from "../ui/icons/TwitterIcon"; // Corrected import
...
const Cards = ({ id, type, title, link, tags, setContentRemoved }: CardProp) => {
    ...
    try {
      await axios
        ...
        .then((response) => {
          ...
          if (setContentRemoved) setContentRemoved(true);
        })
  ...
  return (
    <div
      className={`w-96 bg-white p-4 rounded-md gap-3 flex flex-col shadow-md ${
        type === "youtube" ? "h-96  " : "h-96 overflow-y-hidden hover:h-auto "
      }`}
    >
      <div className="flex items-center justify-between">
        <span>
          {type === "youtube" && <YoutubeIcon />} {type === "tweet" && <TwitterIcon />}
        </span>
